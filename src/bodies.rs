//! Pure combat-body construction + sizing math (the force-sizing primitives + the static template
//! bodies). Lives here so the bot, the sim, and the eval all build/size the SAME bodies with one
//! implementation (no fork) — pure over `screeps-game-api` value types + the engine's mechanics
//! constants, no `game::*`.
//!
//! Two body families live here:
//! - **force-sized** — `CombatBodySpec` (the output of the `force_sizing` solver) → `build_combat_body`
...
//!   counts.
//! - **static templates** — the predefined repeat-template bodies (`solo_defender_body` … `hauler_body`,
//!   the boosted/SK/power-bank variants) the `composition::BodyType` selector maps each slot to, plus
//!   `assemble_combat_body`/`sized_defender_body`/`sized_healer_body` and the `boosts` compound table.

use crate::spawning::SpawnBodyDefinition;
...
// ── Static template bodies (the `composition::BodyType` selector targets) ────
...
pub fn drain_body_for_tower_dps(max_energy: u32, tower_damage_per_tick: f32) -> SpawnBodyDefinition<'static> {
    let min_heal = drain_heal_parts_for_dps(tower_damage_per_tick);
    if min_heal > 13 {
...
mod tests {
    use super::*;
    use crate::spawning::create_body;
